...
names_path = os.path.join('yolo', 'pika.names')
LABELS = open(names_path).read().strip().split("\n")
COLORS = np.random.randint(0, 255, size=(len(LABELS), 3), dtype="uint8")

weights_path = os.path.join('yolo', 'pika_best.weights')
cfg_path = os.path.join('yolo', 'pika.cfg')
...
def results(filename): 
    ...
    if len(class_ids) > 0:
        ...
        return render_template('results.html', labels='no cereals', old_filename=filename, filename=filename) 
